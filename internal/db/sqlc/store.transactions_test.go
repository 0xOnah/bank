...
func TestTransferTx(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		fmt.Println(">> tx:", 1+i, fromAccount.Balance, toAccount.Balance) //checking account for each transaction
		diff1 := account1.Balance - fromAccount.Balance
		diff2 := toAccount.Balance - account2.Balance
		require.Equal(t, diff1, diff2)
		require.True(t, diff1 > 0)         //ensuring the difference is a positive number
