...
func (t *TransferService) CreateTransferTX(ctx context.Context, arg entity.CreateTransferInput, currency string) (*entity.TransferTxResult, error) {
	//from
	account1, err := t.validateAccount(ctx, arg.FromAccountID, currency)
	if err != nil {
		return nil, err
	}
	//balance check
	if account1.Balance < arg.Amount {
		return nil, NewAppError(ErrBadRequest, fmt.Sprintf("you account banance is low: current value %v", account1.Balance), nil)
	}
	//to
